...
<div id="chatbot-widget" class="fixed bottom-0 right-0 z-[9999] text-zinc-900">
    <div x-data="{
        ...
        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            class="fixed bottom-[100px] right-6 h-[600px] w-[380px] overflow-hidden rounded-3xl bg-white shadow-[0_30px_60px_-20px_rgba(0,114,96,0.3),0_0_0_1px_rgba(0,114,96,0.1)]">
            ...
            <div x-show="!chatting" class="flex h-full flex-col">
                ...
                <div class="shrink-0 bg-linear-to-br from-[#0a0a0a] via-[#17494D] to-brand px-4 pb-4 pt-4">
                    <div class="flex items-center gap-2.5">
                        <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-linear-to-br from-[#00A983] to-brand shadow-[0_4px_10px_rgba(45,175,45,0.3)]">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="h-[5px] w-[5px] rounded-full bg-green-400 shadow-[0_0_6px_#4ade80] animate-[chatbot-pulse_2s_infinite]"></span>
                ...
                {{-- Conversations list --}}
                <div class="flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-brand/30">
                    <template x-for="(conv, idx) in conversations" :key="conv.id">
                        <button @click="openConversation(idx)" type="button"
                            class="group flex w-full items-start gap-3 border-b border-gray-100 px-4 py-3 text-left transition hover:bg-[rgba(45,175,45,0.04)]">
                            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg"
                                :class="idx === 0 ? 'bg-[rgba(45,175,45,0.15)]' : 'bg-[rgba(45,175,45,0.06)]'">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                    :class="idx === 0 ? 'text-brand' : 'text-brand/60'">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="truncate text-[11px] leading-snug text-zinc-500">
                                    ...
                                    <span x-text="(conv.messages.filter(m => m.type==='bot').slice(-1)[0]?.text || '').replace(/<[^>]*>/g,'').substring(0,45)"></span>
                                </p>
                            </div>
                            <svg class="mt-1.5 h-3.5 w-3.5 shrink-0 text-zinc-300 transition group-hover:text-brand" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    ...
                    <template x-if="conversations.length === 0">
                        <div class="flex h-full flex-col items-center justify-center px-8 py-12">
                            <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-linear-to-br from-[rgba(45,175,45,0.1)] to-[rgba(45,175,45,0.03)]">
                ...
                <div class="shrink-0 px-4 pb-3 pt-1">
                    <button @click="newConversation()" type="button"
                        class="flex w-full items-center justify-center gap-1.5 rounded-xl bg-linear-to-br from-[#00A983] to-brand py-2.5 text-xs font-bold text-white shadow-[0_6px_16px_-4px_rgba(45,175,45,0.4),inset_0_1px_0_rgba(255,255,255,0.15)] transition hover:scale-[1.02] hover:shadow-[0_10px_22px_-5px_rgba(45,175,45,0.5)] active:scale-[0.97]">
                ...
                <div class="shrink-0 border-t border-gray-200 bg-gray-50 py-2">
                    <p class="flex items-center justify-center gap-1.5 text-[9px] font-semibold tracking-wide text-zinc-400">
                        <span class="h-[5px] w-[5px] rounded-full bg-[#2DAF2D] opacity-50"></span>
                        BUILT WITH HELPDESK
                    </p>
                </div>
            </div>
            ...
            <div x-show="chatting" class="flex h-full flex-col">
                {{-- Chat header --}}
                <div class="flex shrink-0 items-center gap-2.5 bg-linear-to-br from-[#0a0a0a] via-[#17494D] to-brand px-4 py-2.5">
                    <button @click="backToHome()" type="button" class="rounded-lg p-1 text-white/50 transition hover:bg-white/10 hover:text-white">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
                        </svg>
                    </button>
                    <div class="flex h-7 w-7 items-center justify-center rounded-lg bg-linear-to-br from-[#00A983] to-brand shadow-[0_3px_8px_rgba(45,175,45,0.25)]">
                    ...
                    <button @click="close()" type="button" class="rounded-lg p-1 text-white/50 transition hover:bg-white/10 hover:text-white">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                ...
                {{-- Chat messages body --}}
                <div class="flex flex-1 flex-col overflow-y-auto scroll-smooth bg-gray-50/50 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-brand/30" x-ref="chatBody">
                    <div class="flex justify-center px-4 pt-2 pb-1">
                        <span class="rounded-full bg-black/[0.04] px-2 py-0.5 text-[9px] font-medium text-zinc-400 backdrop-blur-sm"
                            ...
                            <div>
                                ...
                                <template x-if="msg.type === 'bot'">
                                    <div class="flex items-end gap-1.5">
                                        <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-linear-to-br from-[#00A983] to-brand">
                                        ...
                                        <div class="flex max-w-[78%] flex-col gap-0.5">
                                            <div class="rounded-xl rounded-bl-md bg-white px-3 py-2 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_0_0_1px_rgba(0,0,0,0.02)]">
                                                <p class="text-xs leading-relaxed text-zinc-800" x-html="msg.text"></p>
                                            </div>
                                ...
                                <template x-if="msg.type === 'user'">
                                    <div class="flex justify-end">
                                        <div class="flex max-w-[78%] flex-col items-end gap-0.5">
                                            <div class="rounded-xl rounded-br-md bg-linear-to-br from-[#00A983] to-brand px-3 py-2 shadow-[0_3px_10px_rgba(45,175,45,0.2)]">
                        ...
                        <div class="flex items-end gap-1.5" x-show="sending">
                            ...
                            <div class="rounded-xl rounded-bl-md bg-white px-3 py-2.5 shadow-[0_1px_3px_rgba(0,0,0,0.03),0_0_0_1px_rgba(0,0,0,0.02)]">
                                <div class="flex gap-1">
                                    <span class="h-[5px] w-[5px] rounded-full bg-[#2DAF2D] opacity-80 animate-[chatbot-bounce_1.2s_infinite]"></span>
                                    <span class="h-[5px] w-[5px] rounded-full bg-[#2DAF2D] opacity-80 animate-[chatbot-bounce_1.2s_infinite_150ms]"></span>
                                    <span class="h-[5px] w-[5px] rounded-full bg-[#2DAF2D] opacity-80 animate-[chatbot-bounce_1.2s_infinite_300ms]"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="shrink-0 border-t border-gray-200 bg-white px-4 py-2.5">
                    <div class="flex items-center gap-2">
                        <input type="text" x-model="userInput" x-on:keydown.enter.prevent="sendMessage(userInput)"
                            ...
                            class="flex-1 rounded-[10px] border border-gray-200 bg-gray-50 px-3 py-2 text-xs text-zinc-900 outline-none transition placeholder:text-zinc-400 focus:border-[#2DAF2D] focus:bg-white focus:ring-2 focus:ring-[rgba(45,175,45,0.3)] disabled:opacity-50" />
                        <button type="button" x-on:click="sendMessage(userInput)"
                            :disabled="!userInput.trim() || sending"
                            class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-linear-to-br from-[#00A983] to-brand text-white shadow-[0_2px_8px_rgba(45,175,45,0.3)] transition active:scale-90 disabled:opacity-30">
        ...
        <button @click="toggle()" type="button"
            class="fixed bottom-6 right-6 z-[9999] flex h-[52px] w-[52px] items-center justify-center rounded-2xl bg-linear-to-br from-[#17494D] to-brand shadow-[0_8px_24px_-6px_rgba(0,114,96,0.5),0_0_0_2px_rgba(0,114,96,0.2)] transition-all duration-200 hover:scale-[1.06] hover:shadow-[0_12px_28px_-6px_#0f2b0f,0_0_0_2px_#2DAF2D] active:scale-95">
            <svg x-show="!open" class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 0 1-.825-.242m9.345-8.334a2.126 2.126 0 0 0-.476-.095 48.64 48.64 0 0 0-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0 0 11.25 3c-2.115 0-4.198.137-6.24.402-1.608.209-2.76 1.614-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155" />
            </svg>
            <svg x-show="open" x-cloak class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
