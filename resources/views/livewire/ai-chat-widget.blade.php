<div>
    ...
    <div x-data="{
        ...
        @open-chatbot-widget.window="openFromShortcut()" x-init="$watch('isOpen', value => { if (value) scrollToBottom() })" id="chatbot-widget"
        ...
        <div x-ref="chatPanel" x-show="visible" x-cloak wire:ignore.self style="transform: translateY(100%); opacity: 0;"
            ...
            @if (!$chatting)
                {{-- Header --}}
                <div
                    class="cw-shimmer relative shrink-0 bg-linear-to-br from-[#0a0a0a] via-[#17494D] to-[#2DAF2D] px-4 pb-4 pt-4 overflow-hidden">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span
                                    class="h-[5px] w-[5px] rounded-full bg-green-400 shadow-[0_0_6px_#4ade80] animate-pulse"></span>
                                <p class="text-[10px] text-white/50">Online now</p>
                            </div>
                        </div>
                        <button @click="toggle()" type="button"
                            class="rounded-lg p-1 text-white/40 transition hover:bg-white/10 hover:text-white">
                ...
                {{-- Conversation list --}}
                <div
                    class="cw-animated-bg flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#2DAF2D]/30">
                    @if (empty($conversations))
                        <div class="flex h-full flex-col items-center justify-center px-8 py-12">
                            <div
                                class="cw-float mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-linear-to-br from-[rgba(45,175,45,0.1)] to-[rgba(45,175,45,0.03)] text-[#2DAF2D]">
                    ...
                    @else
                        @foreach ($conversations as $conv)
                            <button wire:click="selectConversation('{{ $conv['id'] }}')" type="button"
                                ...
                                <div
                                    class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-green-100 bg-white shadow-sm text-[#2DAF2D]">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
            ...
            @else
                ...
                {{-- Chat header --}}
                <div
                    class="cw-shimmer relative flex shrink-0 items-center gap-2.5 bg-linear-to-br from-[#0a0a0a] via-[#17494D] to-[#2DAF2D] px-4 py-2.5 overflow-hidden">
                ...
                <div x-ref="chatContainer"
                    class="cw-animated-bg flex-1 overflow-y-auto scroll-smooth p-4 flex flex-col gap-2 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#2DAF2D]/30">
                    ...
                    @foreach ($messages as $index => $msg)
                        @if ($msg['role'] === 'user')
                            <div class="flex justify-end cw-msg-enter" style="animation-delay: {{ $index * 40 }}ms">
                                <div class="flex max-w-[80%] flex-col items-end gap-0.5">
                                    <div
                                        class="rounded-xl rounded-br-md bg-linear-to-br from-[#00A983] to-[#2DAF2D] px-3 py-2 shadow-[0_3px_10px_rgba(45,175,45,0.2)]">
                        ...
                        @else
                            <div class="flex items-end gap-1.5 cw-msg-enter"
                                ...
                                <div
                                    class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg p-0.5 text-[#2DAF2D]">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
                                ...
                                <div class="flex max-w-[80%] flex-col gap-0.5">
                                    <div
                                        ...
                                        <p class="text-xs leading-relaxed text-zinc-800 whitespace-pre-wrap">
                                            {{ preg_replace('/\*{1,2}([^*]+)\*{1,2}/', '$1', trim($msg['content'])) }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="flex items-end gap-1.5 cw-msg-enter">
                            ...
                            <div
                                ...
                                <div class="flex gap-1">
                                    <span class="h-[5px] w-[5px] rounded-full bg-[#2DAF2D] opacity-80"
