                ...
                <flux:dropdown>
                    ...
                    <flux:menu class="w-[150px]">
                        <flux:menu.radio.group wire:model.live="statusFilter">
                            ...
                            <flux:menu.radio value="deactivated"
                                class="text-zinc-600 dark:text-zinc-300 hover:!bg-emerald-500 hover:!text-white data-active:!bg-emerald-500 data-active:!text-white dark:hover:!bg-emerald-600 dark:hover:!text-white dark:data-active:!bg-emerald-600 dark:data-active:!text-white">
                                Deactivated</flux:menu.radio>
        ...
        <table class="w-full">
            <thead>
                <tr class="border-b border-zinc-200 dark:border-zinc-800">
                    ...
                    <th class="pl-4 pr-2 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-400 cursor-pointer group hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors"
                    ...
                    <th
                        class="w-20 pl-1 pr-2 py-3 text-right text-xs font-medium uppercase tracking-wider text-zinc-400">
                    </th>
                </tr>
            </thead>
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                @forelse ($this->customers as $customer)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors group cursor-pointer"
                        wire:key="{{ $customer->id }}" wire:click="navigateToDetails({{ $customer->id }})">
                        ...
                        <td class="px-4 py-4 text-sm text-zinc-600 dark:text-zinc-400">
                            <span
                                class="inline-flex items-center justify-center px-2 py-0.5 rounded text-xs font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400">
                        ...
                        <td class="pl-4 pr-2 py-4 text-sm">
                            @if ($customer->is_active)
                                <span
                                    class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                    <span class="w-1 h-1 rounded-full bg-emerald-500"></span>
                                    Active
                                </span>
                            @else
                                <span
                                    class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-700">
                                    <span class="w-1 h-1 rounded-full bg-zinc-400"></span>
                                    Inactive
                                </span>
                            @endif
                        </td>
                        <td class="w-20 pl-1 pr-2 py-4 text-right text-sm" wire:click.stop>
                            <div
                                ...
                                <button
                                    @click="confirmAction($wire, {{ $customer->id }}, 'toggleStatus', 'Are you sure?', 'Are you sure you want to {{ $customer->is_active ? 'deactivate' : 'activate' }} this customer?', 'Yes, {{ $customer->is_active ? 'deactivate' : 'activate' }}!')"
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1">Try allowing more lenient
                                    filters to see results.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
