                        ...
                        @forelse ($this->tickets as $ticket)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/30 transition-colors">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    ...
                                    @endphp
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border {{ $statusBg }}">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if ($ticket->user)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span
                                                class="text-zinc-600 dark:text-zinc-300">{{ $ticket->user->name }}</span>
                                        </div>
                                    @else
                                ...
                                <td class="px-4 py-3 text-right">
                                    <a href="{{ route('details', ['company' => Auth::user()->company->slug, 'ticket' => $ticket]) }}"
                                        wire:navigate
                                        class="inline-flex items-center gap-1 px-3 py-1 bg-zinc-100 dark:bg-zinc-700 hover:bg-zinc-200 dark:hover:bg-zinc-600 text-zinc-700 dark:text-zinc-300 text-xs font-medium rounded-md transition-colors border border-zinc-200 dark:border-zinc-600">
                                        View
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-4 py-12 text-center">
                                    ...
                                    <p class="mt-4 text-zinc-500 dark:text-zinc-400">This customer hasn't submitted any
                                        tickets yet.</p>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="space-y-6">
                @forelse ($this->conversations as $reply)
                    <div
                        ...
                        @else
                            <div
                                class="w-10 h-10 rounded-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center text-zinc-600 dark:text-zinc-300 font-bold flex-shrink-0">
                                {{ substr($customer->name, 0, 1) }}
                            </div>
                        @endif
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-4 mb-2">
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        @else
                                            <span
                                                class="px-2 py-0.5 rounded bg-zinc-200 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 text-[10px] font-medium tracking-wide uppercase">
                                                Customer
                                            </span>
                                        @endif
                                    </div>
                            ...
                            <div
                                class="text-zinc-700 dark:text-zinc-300 prose prose-zinc dark:prose-invert prose-sm max-w-none">
                ...
                @empty
                    <div
                        class="py-12 text-center rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
                        <svg class="mx-auto h-12 w-12 text-zinc-400 dark:text-zinc-600" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z" />
                        </svg>
                        <p class="mt-4 text-zinc-500 dark:text-zinc-400">No conversation history yet.</p>
                    </div>
                @endforelse
            </div>
        @endif
