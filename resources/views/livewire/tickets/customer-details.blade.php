<div>
                    ...
                    <div class="mt-2 flex flex-wrap gap-4 text-sm text-zinc-400">
                        <div class="flex items-center gap-1.5">
                            ...
                            <a href="mailto:{{ $customer->email }}"
                                class="hover:text-teal-400 transition-colors">{{ $customer->email }}</a>
                        </div>
                        @if ($customer->phone)
                            <div class="flex items-center gap-1.5">
                                ...
                                <a href="tel:{{ $customer->phone }}"
                                    class="hover:text-teal-400 transition-colors">{{ $customer->phone }}</a>
                            </div>
                        @endif
                        <div class="flex items-center gap-1.5">
                            <svg class="w-4 h-4 text-zinc-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="border-b border-zinc-800 mb-6 flex gap-6">
        <button wire:click="setTab('tickets')"
            ...
            Tickets
            <span
                class="ml-1.5 px-2 py-0.5 rounded-full text-xs {{ $activeTab === 'tickets' ? 'bg-teal-500/20 text-teal-300' : 'bg-zinc-800 text-zinc-400' }}">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-zinc-900/50 border-b border-zinc-800">
                            <th
                                class="px-4 py-3 text-left text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                                Ticket ID</th>
                            <th
                                class="px-4 py-3 text-left text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                                Subject</th>
                            ...
                            <th
                                ...
                                Created</th>
                            <th
                                class="px-4 py-3 text-right text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                            </th>
                        </tr>
                    </thead>
        ...
        @elseif($activeTab === 'conversations')
            ...
            <div class="space-y-6">
                @forelse ($this->conversations as $reply)
                    <div
                        class="flex gap-4 p-5 rounded-lg border {{ $reply->user_id ? 'bg-zinc-800/50 border-zinc-800' : 'bg-teal-500/5 border-teal-500/10' }}">
                        @if ($reply->user_id)
                            <div
                        ...
                        @else
                            <div
                                class="w-10 h-10 rounded-full bg-zinc-700 flex items-center justify-center text-zinc-300 font-bold flex-shrink-0">
                                {{ substr($customer->name, 0, 1) }}
                            </div>
                        @endif
                            ...
                            <div class="flex items-start justify-between gap-4 mb-2">
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if ($reply->user_id)
                                            <span
                                                class="px-2 py-0.5 rounded bg-teal-500/20 text-teal-400 text-[10px] font-medium tracking-wide uppercase">
                                                Agent
                                            </span>
                                        @else
                                            <span
                                                class="px-2 py-0.5 rounded bg-zinc-700 text-zinc-300 text-[10px] font-medium tracking-wide uppercase">
                                                Customer
                                            </span>
                                        @endif
                                    </div>
                                    <div class="text-xs text-zinc-500 mt-0.5 flex items-center gap-2">
                                        <span>in <a
                                                ...
                                                class="hover:text-teal-400 underline">{{ $reply->ticket->ticket_number }}</a></span>
                                        <span>&bull;</span>
