<div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-2">
        <button type="button" x-on:click="$flux.modal('open-tickets-modal').show()" class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/80">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="p-1.5 bg-blue-500/10 rounded-lg">
                    <svg class="w-4 h-4 text-blue-500 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
                </div>
            </div>
        ...
        <button type="button" x-on:click="$flux.modal('resolved-tickets-modal').show()" class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/80">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="p-1.5 bg-green-500/10 rounded-lg">
                    <svg class="w-4 h-4 text-green-500 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
            </div>
        ...
        <button type="button" x-on:click="$flux.modal('pending-tickets-modal').show()" class="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/80">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Pending Reply</span>
                <div class="p-1.5 bg-yellow-500/10 rounded-lg">
                    <svg class="w-4 h-4 text-yellow-500 dark:text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
            </div>
            <p class="text-3xl font-bold text-zinc-900 dark:text-white">{{ $this->pendingReplyCount }}</p>
        </button>

        <a href="{{ route('notifications', Auth::user()->company->slug) }}" wire:navigate class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 text-left transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Unread Notifications</span>
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-800">
                    ...
                    <a href="{{ route('tickets', Auth::user()->company->slug) }}" wire:navigate class="text-xs text-teal-400 hover:text-teal-300 transition-colors">
                        View all →
                    </a>
                </div>
                               ...
                               class="flex items-center gap-4 px-5 py-3.5 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors group">
                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium border {{ $priorityBg }}">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium border {{ $statusBg }}">
                                            {{ ucfirst(str_replace('_', ' ', $ticket->status)) }}
                                        </span>
                                    </div>
                                    <p class="text-sm text-zinc-700 dark:text-zinc-200 truncate group-hover:text-zinc-900 dark:group-hover:text-white transition-colors">{{ $ticket->subject }}</p>
        ...
        <div class="lg:col-span-2 space-y-6">
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center gap-2 mb-1">
                                            ...
                                            @endphp
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium border {{ $uPriorityBg }}">
                                                {{ ucfirst($ticket->priority) }}
                                            </span>
                                        </div>
                                        <p class="text-sm text-zinc-700 dark:text-zinc-200 truncate">{{ $ticket->subject }}</p>
                                    ...
                                    <button wire:click="assignToMe({{ $ticket->id }})"
                                        ...
                                        <span wire:loading wire:target="assignToMe({{ $ticket->id }})">Assigning…</span>
                                    </button>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden">
                <div class="flex items-center justify-between px-5 py-4 border-b border-zinc-200 dark:border-zinc-800">
                    ...
                    <a href="{{ route('notifications', Auth::user()->company->slug) }}" wire:navigate class="text-xs text-teal-400 hover:text-teal-300 transition-colors">
                        View all →
                    </a>
                </div>
                    ...
                    <div class="divide-y divide-zinc-200 dark:divide-zinc-800">
                        @foreach($this->recentNotifications as $notification)
                            <div class="flex items-start gap-3 px-5 py-3.5">
                                ...
                                @endif
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm {{ is_null($notification->read_at) ? 'font-medium text-zinc-900 dark:text-zinc-100' : 'text-zinc-600 dark:text-zinc-300' }}">
                ...
                <div class="divide-y divide-zinc-200 dark:divide-zinc-800/90">
                    @foreach($this->openTicketsList as $ticket)
                        <div class="py-3 flex items-center justify-between group cursor-pointer" onclick="window.location='{{ route('details', ['company' => Auth::user()->company->slug, 'ticket' => $ticket->ticket_number]) }}'">
                            <div>
                                <p class="text-sm font-medium text-zinc-900 dark:text-white group-hover:text-teal-500 dark:group-hover:text-teal-400 transition-colors">{{ $ticket->subject }}</p>
